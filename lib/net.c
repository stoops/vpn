...
int recw(buff_o *buff) {
	...
	if (buff->leng < 4) {
		...
		if (buff->size < BONE) {
			...
			if ((buff->size < BONE) || (BMAX < buff->size)) {
				return -2;
			}
		}
		leng = (buff->size - buff->leng);
	}
	if ((leng < BONE) || (BMAX < leng)) {
		return -3;
	}
	if (buff->sock < BONE) { return -4; }
	size = read(buff->sock, buff->pbuf, leng);
	if (size < BONE) { return size; }
	buff->leng += size;
	buff->pbuf += size;
