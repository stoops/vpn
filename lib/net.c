...
int inis(int sock, struct sockaddr_in *dest) {
	int erro, fdes, leng = sizeof(struct sockaddr_in);
	if (sock < 1) {
		if ((fdes = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
			printf("[%s] warn inis conn sock\n", gett()); close(fdes);
		} else if ((erro = connect(fdes, (struct sockaddr *)dest, leng)) != 0) {
			printf("[%s] warn inis conn conn\n", gett()); close(fdes);
		} else { sock = fdes; }
	}
	return sock;
}

int sets(cons_l *conn, proc_o *proc, pckt_h *head, char *skey, int *rpwp, int *ssoc, int *csoc, int cidx, int indx, int kind) {
	...
	*sock = fdes;
	...
	proc->sock = csoc;
	proc->cons = conn;
	bzero(&(proc->thro), sizeof(pthread_t));
	return fdes;
}
