	...
	while (stat == 1) {
		...
		for (int x = 0; x < 2; ++x) {
			if ((buff[x].sock > 0) && (FD_ISSET(buff[x].sock, &(rfds)))) {
				size = recw(&(buff[x]));
				if (size < 1) {
					if ((size < 0) && (*rsoc > 0)) { printf("[%s] warn udpc_recv recw [%d] [%d] [%d]\n", gett(), x, size, buff[x].sock); }
					if (x == 0) { fins(esoc, 1); }
					else { fins(rsoc, 1); }
					continue;
				}
				...
				for (int cidx = 0; cidx < NUMC; ++cidx) {
					...
					if (cons[cidx].stat == 1) {
						if ((secs - cons[cidx].last) >= EXPC) {
							printf("[%s] info udpc_recv fins [%d] [%s:%d]\n", gett(), cidx, cons[cidx].adrs[0], cons[cidx].port[0]);
							cons[cidx].stat = 0;
...
void *udpc_send(void *argv) {
		...
		for (int x = 0; x < NUMC; ++x) {
			...
			if (cons[x].stat == 1) {
				if ((secs - cons[x].last) >= EXPC) {
					...
					cons[x].stat = 0;
				} else if ((cons[x].port[0] == bufs[indx].port) && (cadr->sin_addr.s_addr == padr->sin_addr.s_addr)) {
					cidx = x; break;
				}
			}
			if (cons[x].stat == 0) {
	...
	printf("[%s] info udpc_send finz [%d]\n", gett(), stat);
