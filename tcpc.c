...
void *tcpc_recv(void *argv) {
	...
	while (stat == 1) {
		...
		if (argp->stat != 1) {
			if (args->conz != 1) {
				printf("[%s] warn tcpc_recv stat [%d] [%d]\n", gett(), stat, argp->stat);
			}
		...
		for (int x = 0; x < 2; ++x) {
			if ((buff[x].sock > 0) && (FD_ISSET(buff[x].sock, &(rfds)))) {
				...
				if (flag == 0) {
					...
					if (indx < 0) {
						if (olen != ENDL) {
							printf("[%s] warn tcpc_recv indx [%d.%d.%d.%d:%d] [%d] [%d]\n", gett(), head->sadr[3], head->sadr[2], head->sadr[1], head->sadr[0], sprt, indx, stat);
							fcon(buff[x].sock, NUMC + x + 1, head, cryp);
						}
	...
	while (stat == 1) {
		...
		if ((secs - last[1]) >= 9) {
			for (int x = 0; x < NUMC; ++x) {
				...
				if (args->nots != NULL) {
					esoc[idxt] = isoc(esoc[idxt], &(eadr));
				}
				rsoc[idxt] = isoc(rsoc[idxt], &(radr));
			}
			last[1] = secs;
		}
		if ((secs - last[0]) >= 3) {
		...
		if (FD_ISSET(lsoc, &(rfds))) {
			...
			if (args->nots != NULL) {
				esoc[tidx] = isoc(esoc[tidx], &(eadr));
			}
			rsoc[tidx] = isoc(rsoc[tidx], &(radr));
			if (thrr[tidx].stat == 0) {
