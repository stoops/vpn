...
void *tcpc_send(void *argv) {
	...
	while (stat == 1) {
		...
		if ((secs - cons->ping[0]) >= (MGMC * 3)) {
			dsoc = *(cons->sock);
			fcon(dsoc, tidx, head, cryp, CHKL);
			cons->ping[0] = secs;
		}
	}
			...
			for (int x = 0; x < NUMC; ++x) {
				if (cons[x].stat == 1) {
					if (((secs - cons[x].ping[0]) >= EXPP) || ((secs - cons[x].ping[1]) >= EXPP)) {
						printf("[%s] warn tcpc_mgmt ping [%d][%ld] [%ld][%ld]\n", gett(), x, secs, cons[x].ping[0], cons[x].ping[1]);
						cons[x].stat = -3;
					}
				}
			}
...
void tcpc_serv(args_o *args) {
	...
	while (stat == 1) {
		...
		if (FD_ISSET(lsoc, &(rfds))) {
			...
			csoc[indx] = fdes;
			cons[indx].ping[0] = secs;
			cons[indx].ping[1] = secs;
			cons[indx].last = secs;
			thrr[tidx].indx = tidx;
			thrs[indx].indx = tidx;
			thrr[tidx].stat = 1;
