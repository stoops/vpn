	...
	while (stat == 1) {
		...
		for (int x = 0; x < 2; ++x) {
			if ((buff[x].sock > 0) && (FD_ISSET(buff[x].sock, &(rfds)))) {
				size = recw(&(buff[x]));
				if (size < 1) {
					if ((size < 0) && (*rsoc > 0)) { printf("[%s] warn tcpc_recv recw [%d] [%d] [%d]\n", gett(), x, size, buff[x].sock); }
...
void *tcpc_send(void *argv) {
	...
	int leng, size, csoc, dsoc, midx, tidx, fdes, fmax;
	int erro = 1, stat = 1;
	int hlen = sizeof(pckt_h);
	int maxl = (BTCP + HEDL);
	char *prot = "tcp";
	...
	while (stat == 1) {
		...
		if (FD_ISSET(csoc, &(rfds))) {
			...
			if (dsoc < 1) {
				...
				if (leng < BONE) {
					//if (leng < 0) { printf("[%s] erro tcpc_send read [%d] [%d]\n", gett(), leng, csoc); }
					stat = leng; break;
				}
				size = (hlen + leng);
	...
	while (stat == 1) {
		...
		if ((secs - last) >= 5) {
			for (int x = 0; x < NUMC; ++x) {
				...
				if (args->nots != NULL) {
					esoc[idxt] = inis(esoc[idxt], &(eadr));
				}
				rsoc[idxt] = inis(rsoc[idxt], &(radr));
			}
			last = secs;
		}
		indx = -1;
		...
		if (FD_ISSET(lsoc, &(rfds))) {
			...
			if (args->nots != NULL) {
				esoc[tidx] = inis(esoc[tidx], &(eadr));
			}
			rsoc[tidx] = inis(rsoc[tidx], &(radr));
			if (thrr[tidx].stat == 0) {
