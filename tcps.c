...
void *tcps_send(void *argv) {
	...
	while (stat == 1) {
		if (argp->stat != 1) {
			if (args->conz != 1) {
				printf("[%s] warn tcps_send stat [%d] [%d] [%d]\n", gett(), indx, stat, argp->stat);
			}
		...
		if ((cidx < 0) || (proc[cidx].stat != 1) || (cons[cidx].stat != 1)) {
			if (olen != ENDL) {
				printf("[%s] warn tcps_send indx [%d.%d.%d.%d:%d] [%d] [%d]\n", gett(), head->sadr[3], head->sadr[2], head->sadr[1], head->sadr[0], sprt, cidx, stat);
				fcon(buff.sock, NUMC + 1, head, cryp);
			}
