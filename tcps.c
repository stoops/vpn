...
void *tcps_recv(void *argv) {
	...
	printf("[%s] info tcps_recv fins [%d] [%d] [%d]\n", gett(), stat, cidx, ssoc);
...
void *tcps_send(void *argv) {
	...
	while (stat == 1) {
		...
		if ((cidx < 0) || (proc[cidx].stat != 1) || (cons[cidx].stat != 1)) {
			if (olen != ENDL) {
				...
				if (cidx > -1) { last = cidx; }
				if (proc[last].stat == 1) { proc[last].stat = 9; }
				if (cons[last].stat == 1) { cons[last].stat = 9; }
			}
		} else {
			if (olen == ENDL) {
				if (proc[cidx].stat == 1) { proc[cidx].stat = 9; }
				if (cons[cidx].stat == 1) { cons[cidx].stat = 9; }
			} else {
	...
	for (int x = 0; x < NUMC; ++x) {
		if (cons[x].indx != indx) { continue; }
		printf("[%s] info tcps_send stop [%d][%d] [%d][%d]\n", gett(), indx, x, proc[x].stat, cons[x].stat);
		if (proc[x].stat == 1) { proc[x].stat = 9; }
		if (cons[x].stat == 1) { cons[x].stat = 9; }
	}
	sleep(9);
	for (int x = 0; x < NUMC; ++x) {
		if (cons[x].indx != indx) { continue; }
		if ((proc[x].stat > 0) || (cons[x].stat > 0)) {
			printf("[%s] warn tcps_send endp [%d][%d] [%d][%d]\n", gett(), indx, x, proc[x].stat, cons[x].stat);
		}
	}
