...
void *devo(void *argv)
{
	...
	int *alen;
	unsigned char *buff = malloc(MAXA * sizeof(unsigned char));
	unsigned char *pntr;
	struct thrp *thra;
	...
	while (1)
	{
		...
		alen = thra->alen;
		leng = *alen; *alen = ZERO;
		if ((leng < 1) || (maxl < leng)) { STAT = (-1 * ((5 * 10) + indx)); break; }
		bcopy(thra->arry, buff, leng);
	...
	while (1)
	{
		...
		for (int z = 0; z < MAXR; ++z)
		{
			...
			if (FD_ISSET(TUNF[indx], &rfds))
			{
				...
				if (leng < 1) { STAT = (-1 * ((5 * 10) + indx)); break; }
				leng = (leng + OFFS);
				pack(bptr, leng);
				llen = (llen + leng);
				bptr = (bptr + leng);
