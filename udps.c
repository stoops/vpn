...
void *udps_send(void *argv) {
	...
	while (stat == 1) {
		...
		size = recw(&(buff));
		if (size < 1) {
			if (size < 0) { printf("[%s] warn udps_send recw [%d] [%d] [%d]\n", gett(), indx, size, buff.sock); }
			stat = size; break;
		}
		size = buff.size;
		pntr = buff.buff;
		...
		for (int x = 0; x < NUMC; ++x) {
			...
			if ((cons[x].stat == 0) && (proc[x].stat == 0)) {
				...
				if ((secs - cons[x].last) >= EXPC) {
					printf("[%s] info udps_send fins [%d] [%s:%d]\n", gett(), x, cons[x].adrs[0], cons[x].port[0]);
					proc[x].stat = 9;
				} else if ((cprt == sprt) && (cadr->sin_addr.s_addr == padr->sin_addr.s_addr)) {
					cidx = x; break;
				}
			}
		}
	...
	for (int x = 0; x < NUMC; ++x) {
		if ((cons[x].indx == indx) && (cons[x].stat > 0)) {
			printf("[%s] warn udps_send endp conn [%d][%d] [%d]\n", gett(), indx, x, cons[x].stat);
		}
		if ((proc[x].indx == indx) && (proc[x].stat > 0)) {
			printf("[%s] warn udps_send endp proc [%d][%d] [%d]\n", gett(), indx, x, proc[x].stat);
		}
	}

	argp->stat = -1;

	printf("[%s] info udps_send finz [%d] [%d]\n", gett(), stat, indx);
