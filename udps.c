...
void *udps_send(void *argv) {
	...
	while (stat == 1) {
		size = recw(&(buff));
		if (size < 1) {
			printf("[%s] warn udps_send size [%d] [%d]\n", gett(), size, buff.sock);
			stat = 0; break;
			...
			if (erro > 0) { pntr = temp; size = erro; }
			if (erro < 0) { printf("[%s] warn udps_send ciph [%d]\n", gett(), erro); }
			else {
				bcopy(pntr, head, hlen);
				UPACK16(leng, head->leng);
				leng -= hlen; pntr += hlen;
				if ((leng < 1) || (BTCP < leng)) { printf("[%s] warn udps_send head [%d]\n", gett(), leng); }
				else {
					secs = time(NULL);
